...
def get_user_uk(cookie, tokens):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:get_user_uk 42')
    '''获取用户的uk'''
...
def get_user_info(tokens, uk):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:get_user_info 55')
    '''获取用户的部分信息.
...
def list_share(cookie, tokens, uk, page=1):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:list_share 75')
    '''获取用户已经共享的所有文件的信息
...
def list_share_files(cookie, tokens, uk, shareid, dirname, page=1):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:list_share_files 105')
    '''列举出用户共享的某一个目录中的文件信息
...
def list_share_single_file(cookie, tokens, uk, shareid):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:list_share_single_file 140')
    '''获取单独共享出来的文件.
    ...
    def parse_share_page(content):
        print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:parse_share_page 147')
...
def enable_share(cookie, tokens, fid_list):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:enable_share 190')
    '''建立新的分享.
...
def disable_share(cookie, tokens, shareid_list):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:disable_share 216')
    '''取消分享.
...
def verify_share_password(uk, shareid, pwd, vcode=''):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:verify_share_password 237')
    '''验证共享文件的密码.
...
def get_share_uk_and_shareid(cookie, url):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:get_share_uk_and_shareid 264')
    '''从共享链接中提示uk和shareid.
    ...
    def parse_share_uk(content):
        print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:parse_share_uk 276')
        '''代码片段如下:
    ...
    def parse_uk_from_url(url):
        print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:parse_uk_from_url 291')
...
def get_share_dirname(url):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:get_share_dirname 323')
    '''从url中提取出当前的目录'''
...
def get_share_url_with_dirname(uk, shareid, dirname):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:get_share_url_with_dirname 332')
    '''得到共享目录的链接'''
    return ''.join([
           const.PAN_URL, 'wap/link',
           '?shareid=', shareid,
...
def share_transfer(cookie, tokens, shareid, uk, filelist, dest, upload_mode):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:share_transfer 342')
    '''
    将其他用户的文件保存到自己网盘里.
...
def list_inbox(cookie, tokens, start=0, limit=20):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:list_inbox 375')
    '''获取收件箱里的文件信息.'''
    url = ''.join([
        const.PAN_URL,
...
def list_trash(cookie, tokens, path='/', page=1, num=100):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:list_trash 393')
    '''获取回收站的信息.
...
def restore_trash(cookie, tokens, fidlist):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:restore_trash 420')
    '''从回收站中还原文件/目录.
...
def delete_trash(cookie, tokens, fidlist):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:delete_trash 442')
...
def clear_trash(cookie, tokens):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:clear_trash 465')
    '''清空回收站, 将里面的所有文件都删除.'''
    url = ''.join([
        const.PAN_API_URL,
...
def list_dir_all(cookie, tokens, path):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:list_dir_all 483')
...
def list_dir(cookie, tokens, path, page=1, num=100):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:list_dir 496')
...
def mkdir(cookie, tokens, path):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:mkdir 521')
    '''创建一个目录.
...
def delete_files(cookie, tokens, filelist):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:delete_files 546')
    '''批量删除文件/目录.
...
def rename(cookie, tokens, filelist):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:rename 567')
    '''批量重命名目录/文件.
...
def move(cookie, tokens, filelist):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:move 592')
    '''移动文件/目录到新的位置.
...
def copy(cookie, tokens, filelist):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:copy 616')
    '''复制文件/目录到新位置.
...
def get_category(cookie, tokens, category, page=1):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:get_category 641')
...
def get_download_link(cookie, tokens, path):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:get_download_link 672')
    '''在下载之前, 要先获取最终的下载链接.
...
def stream_download(cookie, tokens, path):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:stream_download 696')
    '''下载流媒体文件.
...
def get_streaming_playlist(cookie, path, video_type='M3U8_AUTO_480'):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:get_streaming_playlist 714')
    '''获取流媒体(通常是视频)的播放列表.
...
def upload(cookie, source_path, path, upload_mode):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:upload 754')
    '''上传一个文件.
...
def rapid_upload(cookie, tokens, source_path, path, upload_mode):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:rapid_upload 783')
    '''快速上传'''
    ondup = const.UPLOAD_ONDUP[upload_mode]
...
def slice_upload(cookie, data):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:slice_upload 810')
    '''分片上传一个大文件
...
def create_superfile(cookie, path, block_list):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:create_superfile 832')
...
def get_metas(cookie, tokens, filelist, dlink=True):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:get_metas 855')
    '''获取多个文件的metadata.
...
def search(cookie, tokens, key, path='/'):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:search 887')
    '''搜索全部文件, 根据文件名.
...
def cloud_add_link_task(cookie, tokens, source_url, save_path,
                        vcode='', vcode_input=''):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:cloud_add_link_task 909')
    '''新建离线下载任务.
...
def cloud_add_bt_task(cookie, tokens, source_url, save_path, selected_idx,
    ...
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:cloud_add_bt_task 947')
...
def cloud_query_sinfo(cookie, tokens, source_path):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:cloud_query_sinfo 995')
...
def cloud_query_magnetinfo(cookie, tokens, source_url, save_path):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:cloud_query_magnetinfo 1016')
    '''获取磁链的信息.
...
def cloud_list_task(cookie, tokens, start=0):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:cloud_list_task 1044')
    '''获取当前离线下载的任务信息
...
def cloud_query_task(cookie, tokens, task_ids):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:cloud_query_task 1065')
...
def cloud_cancel_task(cookie, tokens, task_id):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:cloud_cancel_task 1088')
    '''取消离线下载任务.
...
def cloud_delete_task(cookie, tokens, task_id):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:cloud_delete_task 1110')
...
def cloud_clear_task(cookie, tokens):
    print('/usr/local/lib/python3.4/dist-packages/bcloud/pcs.py:cloud_clear_task 1131')
    '''清空离线下载的历史(已经完成或者取消的).'''
    url = ''.join([
        const.PAN_URL,
